...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        if debug_timing:
            print(f"[opd_timing][rank{rank}] forward_micro start response_length={response_length}", flush=True)
        multi_modal_inputs = {}
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if debug_timing:
                    print(f"[opd_timing][rank{rank}] unpad={time.perf_counter() - t_unpad:.3f}s nnz={input_ids_rmpad.shape[0]}", flush=True)
                ...
                if debug_timing:
                    print(f"[opd_timing][rank{rank}] before_actor_forward shape={tuple(input_ids_rmpad.shape)}", flush=True)
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if need_topk:
                        ...
                        if debug_timing:
                            torch.cuda.synchronize()
                            print(f"[opd_timing][rank{rank}] target_log_probs={time.perf_counter() - t_logp:.3f}s", flush=True)
                    else:
                        log_probs = logprobs_from_logits(
                            logits=logits_rmpad,
                            labels=input_ids_rmpad_rolled,
                    ...
                    if need_topk:
                        if student_top_k_ids is not None:
                             ...
                             topk_logits, topk_ids = torch.topk(logits_rmpad, k=top_k, dim=-1)

                        t_topk_logp = time.perf_counter()
                        if student_top_k_ids is not None:
                             topk_logits = logits_rmpad.gather(dim=-1, index=topk_ids)
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if need_topk:
                        ...
                        if student_top_k_ids is not None:
                             topk_logits = logits.gather(dim=-1, index=topk_ids)
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        if debug_timing and rank == 0:
            print(f"[opd_timing] compute_log_prob start micro_batches={len(micro_batches)} calculate_entropy={calculate_entropy}", flush=True)
        log_probs_lst = []
        entropy_lst = []
        topk_ids_lst = []
        topk_log_probs_lst = []
        ...
        for mb_idx, micro_batch in enumerate(micro_batches):
            ...
            micro_batch = micro_batch.to(get_device_id())
            if debug_timing and rank == 0:
                print(f"[opd_timing] mb={mb_idx} to_device={time.perf_counter() - t0:.3f}s", flush=True)
            model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
            ...
            if debug_timing and rank == 0:
                torch.cuda.synchronize()
                print(f"[opd_timing] mb={mb_idx} forward_micro={time.perf_counter() - t1:.3f}s", flush=True)
            # Keep on GPU to avoid expensive CPU-GPU transfer for large top-k
        ...
        if debug_timing and rank == 0:
            print(f"[opd_timing] concat_log_probs={time.perf_counter() - t_concat:.3f}s", flush=True)
        entropys = None
        if calculate_entropy:
        ...
        if use_dynamic_bsz:
            ...
            if top_k > 0:
                ...
                topk_log_probs_tensor = restore_dynamic_batch(topk_log_probs_tensor, batch_idx_list)
            if debug_timing and rank == 0:
                print(f"[opd_timing] restore_dynamic={time.perf_counter() - t_restore:.3f}s", flush=True)
