...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        opd_sampling_params = prompts.meta_info.get("opd_sampling_params", None)
        opd_use_tqdm = False
        per_prompt_max_tokens = None
        if opd_sampling_params is not None:
            kwargs = dict(opd_sampling_params)
            opd_use_tqdm = bool(kwargs.pop("use_tqdm", True))
            per_prompt_max_tokens = kwargs.pop("per_prompt_max_tokens", None)
            kwargs["logprobs"] = None
            if "opd_per_prompt_max_tokens" in non_tensor_batch:
                per_prompt_max_tokens = [int(x) for x in non_tensor_batch.pop("opd_per_prompt_max_tokens")]
        elif not do_sample:
            kwargs = {
                "best_of": 1,
                "top_p": 1.0,
        ...
        if opd_sampling_params is not None:
            padding_max_length = int(opd_sampling_params.get("max_tokens", self.config.response_length))
        elif is_validate:
            val_max_tokens = self.config.val_kwargs.get("max_tokens", self.config.response_length)
        ...
        with self.update_sampling_params(**kwargs):
            ...
            if per_prompt_max_tokens is not None:
                if len(per_prompt_max_tokens) != len(vllm_inputs):
                    raise RuntimeError(
                        f"per_prompt_max_tokens has {len(per_prompt_max_tokens)} entries for {len(vllm_inputs)} prompts"
                    )
                sampling_params = []
                for max_tokens in per_prompt_max_tokens:
                    per_prompt_params = copy.deepcopy(self.sampling_params)
                    per_prompt_params.max_tokens = int(max_tokens)
                    sampling_params.append(per_prompt_params)

            outputs = self.inference_engine.generate(
                prompts=vllm_inputs,  # because we have already convert it to prompt token id
                sampling_params=sampling_params,
                lora_request=lora_requests,
                use_tqdm=opd_use_tqdm,
            )

            # TODO(sgm): disable logprob when recompute_log_prob is enable
            # if n = 1: (bs, response_length) ; if n > 1: (bs * n, response_length)

            collect_log_probs = self.config.calculate_log_probs and opd_sampling_params is None
            response = []
            rollout_log_probs = []
            ...
            actual_batch_size = response.size(0)
            if actual_batch_size != batch_size:
                if actual_batch_size % batch_size != 0:
                    raise RuntimeError(
                        f"vLLM returned {actual_batch_size} samples for batch_size={batch_size}; cannot align prompts"
                    )
                repeat_times = actual_batch_size // batch_size
                idx = idx.repeat_interleave(repeat_times, dim=0)
                attention_mask = attention_mask.repeat_interleave(repeat_times, dim=0)
                position_ids = position_ids.repeat_interleave(repeat_times, dim=0)
                ...
                for key, value in non_tensor_batch.items():
                    try:
                        if len(value) == batch_size:
                            ...
                            repeated_non_tensor_batch[key] = value
                    except TypeError:
                        repeated_non_tensor_batch[key] = value
