...
def reward_func_eval(
    data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None
):
    """Return float score for verl.trainer.main_eval (reward_func returns dict)."""
    res = reward_func(
        data_source, solution_str, ground_truth, extra_info, sandbox_fusion_url, concurrent_semaphore
    )
    if isinstance(res, dict):
